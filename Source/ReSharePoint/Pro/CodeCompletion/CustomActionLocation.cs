...
namespace ReSharePoint.Pro.CodeCompletion
{
    ...
    public class CustomActionLocation : ItemsProviderOfSpecificContext<SPXmlCodeCompletionContext>
    {
        ...
        private bool IsInvalid(ITreeNode treeNode)
        {
            ...
            if (treeNode is IXmlAttributeValue attributeValue)
            {
                ...
                {
                    if (attribute.Parent is IXmlTagHeader tagHeader && (tagHeader.ContainerName == "CustomActionGroup" || tagHeader.ContainerName == "CustomAction"))
                    {
                        result = true;
                    }
                }
        ...
        protected override bool AddLookupItems(SPXmlCodeCompletionContext context, IItemsCollector collector)
        {
            ...
            foreach (var spField in TypeInfo.DefaultCustomActionLocations.Where(predicateBuiltIn))
            {
                collector.Add(new LocationLookupItem(prefix, spField, context.Ranges.ReplaceRange, CompletionCaseType._CustomActionLocation));
            }
