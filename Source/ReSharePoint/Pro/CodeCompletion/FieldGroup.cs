...
namespace ReSharePoint.Pro.CodeCompletion
{
    ...
    public class FieldGroup : ItemsProviderOfSpecificContext<SPXmlCodeCompletionContext>
    {
        ...
        private bool IsInvalid(SPXmlCodeCompletionContext context)
        {
            ...
            if (treeNode is IXmlAttributeValue attributeValue)
            {
                if (attributeValue.Parent is IXmlAttribute attribute && attribute.AttributeName == "Group")
                {
                    ...
                    {
                        if (attribute.Siblings().FirstOrDefault(x => x is IXmlAttribute &&  (x as IXmlAttribute).AttributeName == "ID") is IXmlAttribute idAttribute)
        ...
        protected override bool AddLookupItems(SPXmlCodeCompletionContext context, IItemsCollector collector)
        {
            ...
            Func<FieldXmlEntity, bool> predicate = x => context.Tag == null || x.Id != (string)context.Tag;
            ...
            foreach (var group in FieldCache.GetInstance(solution).Items.Where(predicate).Select(x => x.Group).Distinct())
            {
                if (!String.IsNullOrEmpty(group))
                    collector.Add(new FieldGroupLookupItem(prefix, group, context.Ranges.ReplaceRange, CompletionCaseType._FieldGroup));
            }
